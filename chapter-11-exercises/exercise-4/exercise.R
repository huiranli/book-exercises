...
mutate(flights, delay_in_air = arr_delay - dep_delay)
View(flights)
...
flights %>% 
  ...
  summerize(avg_delayed = mean(delayed_in_air, na.rm = TRUE))
  pull(avg_delayed) # display the results

# generates the same results as
to_sea <- filter(flights, dest == "SEA")
View(to_sea)
to_sea <- select(to_sea, orogin, dest, delay_in_air)
